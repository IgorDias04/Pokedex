...
pokemons[0].pokemon.forEach((p) => {
  pokemonHTML += `<div
    data-bs-toggle="modal"
    data-bs-target="#exampleModal"
    pokemonid="${p.id}"
    title="${p.name}"
    class="card border-dark m-1 testePoke"
    style="width: 15rem">
        <img
            src="${p.img}"
            class="card-img-top"
        ...
        <div class="card-body">
            <ul class="list-group list-group-flush">
                <li class="list-group-item nome">Nome: ${p.name}</li>
                <li class="list-group-item">Altura: ${p.height}</li>
                <li class="list-group-item">Peso: ${p.weight}</li>

                 <div class="d-flex justify-content-center my-1">
                 ${p.type
                   .map((tipo) => {
                     return `<span class="badge mx-1 ${tipo.toLowerCase()}">${tipo}</span>`;
                   })
                   .join("")}
                </div>
...
body.innerHTML = pokemonHTML;

document.querySelectorAll(".testePoke").forEach((item) => {
  item.addEventListener("click", () => {
    let pokemonId = item.getAttribute("pokemonid");
    let cur = pokemons[0].pokemon.find(getPokemon);
    cur.next_evolution = cur.next_evolution || [];
    cur.prev_evolution = cur.prev_evolution || [];

    document.querySelector(".modal-body").innerHTML = `
    <div class="card border-dark">
  <img src="${cur.img}" class="card-img-top" alt="${cur.name}">
  <div class="card-body">
    <h5 class="card-title">${cur.name}</h5>
  </div>
  <ul class="list-group list-group-flush">
    <li class="list-group-item">Nome: ${cur.name}</li>
    <li class="list-group-item">Altura: ${cur.height}</li>
    <li class="list-group-item">Peso: ${cur.weight}</li>
    ${cur.prev_evolution
      .map((l) => {
        return `<li class="list-group-item">Evolução Anterior: ${l.name}</li>`;
      })
      .join("")}
    ${cur.next_evolution
      .map((l) => {
        return `<li class="list-group-item">Próxima evolução: ${l.name}</li>`;
      })
      .join("")}
